...
const TopLeft = () => {
  ...
  const projectName = useSyncExternalStore(game.subscribe.bind(game), () => {
    const project = game.project;
    return project && !project.completed && !project.failed
      ? project.projectName
      : "None selected";
  });
  ...
  const projectSize = useSyncExternalStore(game.subscribe.bind(game), () => {
    const project = game.project;
    return project && !project.completed && !project.failed
      ? game.project.projectSize
      : 0;
  });
