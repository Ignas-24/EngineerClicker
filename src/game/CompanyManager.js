export class CompanyManager {
    ...
    startUpkeepTimer() {
        ...
        this.upkeepInterval = setInterval(() => {
            if (this.currentCompany) {
                const upkeepCost = this.currentCompany.upkeep + this.calculateDeveloperUpkeep();
    ...
    hireDeveloper(tier) {
        const totalEmployees = Object.values(this.developers).reduce((sum, count) => sum + count, 0);
    ...
    calculateTotalEfficiency() {
        ...
        let totalEfficiency = Object.entries(this.developers).reduce(
            (total, [tier, count]) => total + (developerTiers[tier]?.efficiency || 0) * count,
            0
        );
        ...
        if (leadCount > 0) {
            totalEfficiency += Object.entries(this.developers).reduce((boostTotal, [tier, count]) => {
    ...
    buyCompany(type) {
        ...
        const completedProjectsThisReset = this.game.projectManager.completedProjectsThisReset;
        ...
        const companyTypes = {
            ...
            medium: {
                ...
                requirements: () => this.currentCompany?.type === "small" && completedProjectsThisReset >= 20,
            },
            large: {
                cost: 100000,
                ...
                requirements: () => this.currentCompany?.type === "medium" && completedProjectsThisReset >= 50,
            },
        };
