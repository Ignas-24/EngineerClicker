...
test.describe("Project flows", () => {
  ...
  test("selecting and cancelling a project resets its state", async ({
    context,
    page,
  }) => {
    ...
    await expect(
      page.getByText("Progress: 0.00, Size: 10, Time left: 100"),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Progress: 0.00, Size: 0, Time left: 0"),
    ).toBeVisible();
  });

  test("completing a project by clicking awards reward and deselects", async ({
    context,
    page,
  }) => {
    ...
    await page
      .getByRole("button", {
        name: "COMPLETE - Size: 5, Deadline: 50, Reward: 7€",
      })
      .click();
    await page.getByRole("button", { name: "Open Available Projects" }).click();
  ...
  test("an active project fails when its timer runs out", async ({
    context,
    page,
  }) => {
